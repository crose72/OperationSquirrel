...
void calc_yaw_target_error(void)
{
    ...
    g_mav_veh_yaw_cmded = g_mav_veh_yaw - yaw_initial;
    ...
    {
        g_yaw_target_error = g_yaw_target - g_mav_veh_yaw_cmded;
    }
    else
    {
...
void dtrmn_follow_vector(void)
{
    ...
    {
        m_vx_cmd = pid_rev.pid3(Kp_x_rev, Ki_x_rev, Kd_x_rev,
                                              g_x_error, 0.0, 0.0,
                                              w1_x_rev, 0.0, 0.0, ControlDim::X, g_dt);
        m_vy_cmd = pid_rev.pid3(Kp_y_rev, Ki_y_rev, Kd_y_rev,
                                    g_y_error, 0.0, 0.0,
                                    w1_y_rev, 0.0, 0.0, ControlDim::Y, g_dt);
        m_yaw_cmd = pid_yaw.pid3(Kp_yaw, Ki_yaw, Kd_yaw,
                                    g_yaw_target_error, 0.0, 0.0,
                                    w1_yaw, 0.0, 0.0, ControlDim::YAW, g_dt);
    }
    else if (g_target_valid && !g_target_too_close)
    {
        m_vx_cmd = pid_forwd.pid3(Kp_x, Ki_x, Kd_x,
                                            g_x_error, 0.0, 0.0,
                                            w1_x, 0.0, 0.0, ControlDim::X, g_dt);
        m_vy_cmd = pid_forwd.pid3(Kp_y, Ki_y, Kd_y,
                                            g_y_error, 0.0, 0.0,
